...
/**
 * Condition to check if Redis/Valkey service is available in VCAP_SERVICES
 * Only processes RedisConfig when Redis/Valkey services are actually present
 */
public class RedisAvailableCondition implements Condition {
    ...
    public boolean matches(ConditionContext context, AnnotatedTypeMetadata metadata) {
        ...
        try {
            ...
            // Check all service keys for redis or valkey
            var fields = root.fields();
            while (fields.hasNext()) {
                var entry = fields.next();
                String serviceKey = entry.getKey().toLowerCase();
                if (serviceKey.contains("redis") || serviceKey.contains("valkey")) {
                    return true; // Found Redis service
                }
                ...
                if (services.isArray()) {
                    for (JsonNode service : services) {
                        ...
                        if (service.has("label")) {
                            ...
                            if (label.contains("redis") || label.contains("valkey")) {
                                return true; // Found Redis service
                            }
                        }
                        // Check name
                        if (service.has("name")) {
                            ...
                            if (name.contains("redis") || name.contains("valkey")) {
                                return true; // Found Redis service
                            }
                        }
                    }
            ...
            // If we can't parse VCAP_SERVICES, assume Redis is not available
            return false;
        }
        
        return false; // No Redis service found
    }
}
